...
2. **點數固定**：每條線固定 2 個端點（`menu: [Point, Point]`）
...
| 8 | 增加轉折點（貝茲曲線） | ⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐ | High | ⚠️ 需先評估 |

#### 功能細節

**8. 增加轉折點（貝茲曲線控制點）** ⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐ 🔥
- **這是最困難的功能，需要完全重構核心型別系統**
- **架構變更**：
  ```typescript
  // 目前
  interface LineState {
  ...
  ```
- **影響範圍**：
  - ❌ `EditorCanvas.tsx` - 渲染邏輯改用 Bezier 曲線
  - ❌ `generator.ts` - 完全重寫 `calculatePathData`
  - ❌ 點對點對應邏輯 - 如何對應不同數量的點？
- **關鍵問題需先確定**：
  1. 是否允許 menu 和 close 有不同數量的點？
  2. 支援的曲線類型？（Cubic/Quadratic/混合）
  3. 點的對應規則？
- **建議**：如果確定要做，必須最先做（會推翻現有功能）
